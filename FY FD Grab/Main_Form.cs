    ...
    public partial class Main_Form : Form
    {
            ...
            if (webBrowser.ReadyState == WebBrowserReadyState.Complete)
            {
                if (e.Url == webBrowser.Url)
                {
                    try
                    {
                        ...
                        if (webBrowser.Url.ToString().Equals("http://cs.ying168.bet/player/list") || webBrowser.Url.ToString().Equals("http://cs.ying168.bet/site/index") || webBrowser.Url.ToString().Equals("http://cs.ying168.bet/player/online") || webBrowser.Url.ToString().Equals("http://cs.ying168.bet/message/platform") || webBrowser.Url.ToString().Equals("http://cs.ying168.bet/playerFund/dptVerify"))
                        {
        ...
        private async Task ___GetPlayerListsRequest()
        {
            ...
            try
            {
                ...
                byte[] result_gettotal = await wc.UploadValuesTaskAsync("http://cs.ying168.bet/playerFund/dptHistoryAjax", "POST", reqparm_gettotal);
        ...
        private void SendMyBot(string message)
        {
            try
            {
                ...
                string text = "Brand:%20-----" + __brand_code + " " + __app + "-----%0AIP:%20192.168.10.252%0ALocation:%20Robinsons%20Summit%0ADate%20and%20Time:%20[" + datetime + "]%0AMessage:%20" + message + "";
        ...
        private async Task ___GetListDepositVerify()
        {
            try
            {
                ...
                byte[] result = await wc.UploadValuesTaskAsync("http://cs.ying168.bet/playerFund/dptVerifyAjax", "POST", reqparm_gettotal);
                string responsebody = Encoding.UTF8.GetString(result);
                var deserializeObject = JsonConvert.DeserializeObject(responsebody);
